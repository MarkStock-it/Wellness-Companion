...
function Consent({ onAccept, onCancel }) {
  ...
  return <div className="mt-5">
    <div className="rounded-2xl bg-clay-light p-4">
      <div className="flex gap-3"><Icon name="shield" className="shrink-0 text-clay"/><div><h3 className="font-bold">Your information leaves this device</h3><p className="mt-2 text-sm leading-relaxed text-inkSoft">When you use AI features, selected profile details, wellness logs, questions, meal photos, or locally extracted lab-report text are sent to the configured provider—OpenAI, Google Gemini, or DeepSeek—for processing. Lab-report images are scanned locally and are not sent to the AI provider.</p></div></div>
    </div>
...
function ApiSetup({ onSave, onCancel }) {
  ...
  function submit(e){
    ...
    onSave({provider,apiKey:apiKey.trim(),visionProvider:provider==='deepseek'?visionProvider:provider,visionApiKey:provider==='deepseek'?visionApiKey.trim():apiKey.trim()});
  }
  return <form onSubmit={submit} className="mt-5 space-y-4">
    <div className="rounded-2xl bg-teal-light p-4"><div className="flex gap-3"><Icon name="shield" className="shrink-0 text-teal-dark"/><div><h3 className="font-bold text-teal-dark">Session-only API access</h3><p className="mt-1 text-sm leading-relaxed text-inkSoft">Your key stays in this browser tab and is cleared when the tab is closed. It is sent through this app only when you request an AI response.</p></div></div></div>
    <Field label="AI provider"><select className="field" value={provider} onChange={e=>setProvider(e.target.value)}><option value="gemini">Google Gemini</option><option value="deepseek">DeepSeek</option><option value="openai">OpenAI</option></select></Field>
    <Field label={`${provider==='gemini'?'Gemini':provider==='deepseek'?'DeepSeek':'OpenAI'} API key`}>
      <div className="relative"><input className="field pr-20 font-mono text-sm" type={showKey?'text':'password'} value={apiKey} onChange={e=>setApiKey(e.target.value)} autoComplete="off" spellCheck="false" placeholder="Paste your API key"/><button type="button" className="absolute inset-y-0 right-2 px-2 text-xs font-bold text-teal-dark" onClick={()=>setShowKey(!showKey)}>{showKey?'Hide':'Show'}</button></div>
    </Field>
    {provider==='deepseek'&&<div className="space-y-4 rounded-2xl border border-line bg-canvas p-4"><div><h3 className="font-bold">Photo analysis provider</h3><p className="mt-1 text-xs leading-relaxed text-inkSoft">DeepSeek is used for chat and summaries. Choose a multimodal provider for meal and lab photos.</p></div><Field label="Vision provider"><select className="field" value={visionProvider} onChange={e=>setVisionProvider(e.target.value)}><option value="gemini">Google Gemini</option><option value="openai">OpenAI</option></select></Field><Field label="Vision API key" optional><input className="field font-mono text-sm" type="password" value={visionApiKey} onChange={e=>setVisionApiKey(e.target.value)} autoComplete="off" spellCheck="false" placeholder="Needed only for photo analysis"/></Field></div>}
    <p className="text-xs leading-relaxed text-inkSoft">For a public production app, use short-lived tokens or a managed server credential instead of distributing permanent provider keys to browsers.</p>
    <div className="flex gap-3"><BigButton type="button" variant="outline" onClick={onCancel}>Cancel</BigButton><BigButton type="submit" disabled={apiKey.trim().length<10}>Use this key</BigButton></div>
  </form>;
}
...
export default function PrototypeShell() {
  ...
  function startAi(){const s=getAiSettings();setProfile(s.profile);setConsent(s.consent);if(!s.profile)setStage('profile');else if(!s.consent)setStage('consent');else if(!s.config?.apiKey)setStage('api');else setChat(true)}
  function saveProfile(data){localStorage.setItem('wc-profile',JSON.stringify(data));setProfile(data);setStage('consent')}
  function accept(){localStorage.setItem('wc-ai-consent',JSON.stringify({accepted:true,version:'prototype-2-multi-provider',at:new Date().toISOString()}));setConsent(true);setStage('api')}
  function saveApiConfig(config){sessionStorage.setItem('wc-ai-session-config',JSON.stringify(config));setStage(null);setChat(true)}
  return <><button onClick={startAi} className="fixed bottom-[88px] right-4 z-40 flex h-14 w-14 items-center justify-center rounded-full bg-teal text-white shadow-[0_10px_30px_rgba(11,75,76,.35)] transition hover:scale-105" aria-label="Open Wellness AI helper"><Smile/></button>
   ...
   <Modal open={stage==='api'} title="Connect your AI" onClose={()=>setStage(null)}><ApiSetup onSave={saveApiConfig} onCancel={()=>setStage(null)}/></Modal>
